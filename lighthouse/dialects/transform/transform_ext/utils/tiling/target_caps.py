...
def is_f32_contraction(op: ir.Operation | ir.OpView) -> bool:
    """True for a contraction accumulating into f32."""
    types = _contraction_operand_types(op)
    return types is not None and isinstance(types[2], ir.F32Type)
...
def generic_parallel_tiles(
    ...
    inner = min(tile_size, _vector_lane_count(target, out_elem))
    return [inner] if len(parallel_dims) == 1 else [1, inner]


def generic_reduction_tiles(tile_size: int) -> list[int]:
    """Default reduction tile for ops without a microkernel profile."""
    return [max(1, tile_size // 16)]
